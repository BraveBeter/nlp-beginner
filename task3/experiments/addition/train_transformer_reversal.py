"""
实验1：反转结果实验 - Transformer

训练 Transformer 模型，使用反转的结果字符串（如 "123+456=975"）。
这可以帮助模型学习从右到左的加法过程（从个位开始）。
================================================================================
"""
...
def main():
    ...
    import json
    exp_results_path = os.path.join(args.log_dir, "transformer_reversal_results.json")
    with open(exp_results_path, 'w', encoding='utf-8') as f:
        json.dump(exp_results, f, indent=2, ensure_ascii=False)

    print(f"\n实验1完成！反转训练的 Transformer 模型结果已保存")
    print("=" * 60)
