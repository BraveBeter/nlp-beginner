"""
Transformer (Encoder-Decoder) 训练脚本 - 加法任务

训练标准的 Encoder-Decoder Transformer 架构来执行加法运算。
================================================================================
使用方法:
...
from data.addition import AdditionTokenizer, AdditionDataGenerator, AdditionDataset, collate_fn_encoder_decoder, load_dataset_from_file
...
def load_data(args) -> tuple:
    ...
    if args.use_cached and os.path.exists(train_path):
        ...
        train_dataset = load_dataset_from_file(train_path)
        val_dataset = load_dataset_from_file(val_path)
        test_dataset = load_dataset_from_file(test_path)
...
def create_model(args, vocab_size: int) -> Transformer:
    """
    创建 Transformer 模型
...
def evaluate_model(args, model, test_loader, tokenizer):
    ...
    for i, (src, tgt, pred) in enumerate(samples, 1):
        # 解码
        src_text = tokenizer.decode(src, skip_special_tokens=False)
        tgt_text = tokenizer.decode(tgt, skip_special_tokens=False)
...
def save_config(args, results):
    ...
    config_path = os.path.join(args.log_dir, f"transformer_{args.dataset_type}_config.json")
